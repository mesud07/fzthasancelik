...
class Merlin_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		if ( is_wp_error( $content ) ) {
			Logger::error( $content->get_error_message(), array( 'url' => $url, 'filename' => $filename ) );

			return $content;
		}

		$saved_file = File_Manager::create_file( $this->download_directory_path . $filename, $content );

		if ( ! empty( $saved_file ) ) {
			return $this->download_directory_path . $filename;
		}

		Logger::error( 'The file was not able to save to disk, while trying to download it', array( 'url' => $url, 'filename' => $filename ) );
	...
	private function get_content_from_url( $url ) {
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'eye-care' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
