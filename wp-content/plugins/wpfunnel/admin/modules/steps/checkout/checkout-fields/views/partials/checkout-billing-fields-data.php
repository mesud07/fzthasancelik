...
foreach($billing_field as $a => $bf){ 
    if($bf['delete'] == 0){
        ...
        if($bf['required'] == true){ 
            $html .= '<input type="checkbox" checked name="field-required" id="'.$a.'-field-required" class="wpfnl_change_required" value="1" data-id="'.$a.'" data-type="billing"/>';
        }else{
            $html .= '<input type="checkbox" name="field-required" id="'.$a.'-field-required" class="wpfnl_change_required" value="1" data-id="'.$a.'" data-type="billing"/>';
        }
        ...
        $html .= '<div class="wpfnl-switcher">';
        if($bf['enable'] == 1){ 
            $html .= '<input type="checkbox" checked name="field-status" id="'.$a.'-field-status" class="wpfnl_change_enable" value="1" data-id="'.$a.'" data-type="billing"/>';						
        }else{
            $html .= '<input type="checkbox" name="field-status" id="'.$a.'-field-status" class="wpfnl_change_enable" value="1" data-id="'.$a.'" data-type="billing"/>';
        }
        $html .= '<label for="'.$a.'-field-status"></label>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="field-item field-action" data-label="Actions :">';
        $html .= '<button type="button" class="edit-field wpfnl_edit_row" title="Edit Field" data-id="'.$a.'" data-type="billing">';
        ...
        $html .= '</svg>';
        $html .= '</button>';
        $html .= '<button type="button" class="delete-checkout-field" title="Delete Field" data-id="'.$a.'"data-type="billing" id="wpfnl_delete">';
