<?php 
foreach($additional_fields as $a => $bf){ 
    if($bf['delete'] == 0){
        $html .= '<div class="checkout__single-field" id="'.$a.'">';
        $html .=  '<div class="field-item field-hamburger">';
        $html .=  '<span class="hamburger-bar"></span>';
        $html .=  '</div>';
        $html .=  '<div class="field-item field-name billing_name" data-label="Name :">'.$a.'</div>';
        $html .=  '<div class="field-item field-type" data-label="Type :">'.$bf['type'].'</div>';
        $html .=  '<div class="field-item field-label billing_label" data-label="Label :">'.$bf['label'].'</div>';
        ...
        if($bf['required'] == true){ 
            $html .= '<input type="checkbox" checked name="field-required" id="'.$a.'-field-required" class="wpfnl_change_required" value="1" data-id="'.$a.'" data-type="additional"/>';
        }else{
            $html .= '<input type="checkbox" name="field-required" id="'.$a.'-field-required" class="wpfnl_change_required" value="1" data-id="'.$a.'" data-type="additional"/>';
        }
        ...
        $html .= '<div class="wpfnl-switcher">';
        if($bf['enable'] == 1){ 
            $html .= '<input type="checkbox" checked name="field-status" id="'.$a.'-field-status" class="wpfnl_change_enable" value="1" data-id="'.$a.'" data-type="additional"/>';						
        }else{
            $html .= '<input type="checkbox" name="field-status" id="'.$a.'-field-status" class="wpfnl_change_enable" value="1" data-id="'.$a.'" data-type="additional"/>';
        }
        $html .= '<label for="'.$a.'-field-status"></label>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="field-item field-action" data-label="Actions :">';
        $html .= '<button type="button" class="edit-field wpfnl_edit_row" title="Edit Field" data-id="'.$a.'" data-type="additional">';
        ...
        $html .= '</svg>';
        $html .= '</button>';
        $html .= '<button type="button" class="delete-checkout-field" title="Delete Field" data-id="'.$a.'" data-type="additional" id="wpfnl_delete">';
