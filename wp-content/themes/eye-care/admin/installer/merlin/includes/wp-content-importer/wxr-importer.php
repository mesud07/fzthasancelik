...
use EyeCareSpace\Admin\Installer\Merlin\Includes\WpContentImporter\WXR_Import_Info;
...
class WXR_Importer extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'eye-care' ) );
		}
	 ...
	 *
	 * @return WXR_Import_Info|WP_Error
	 */
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						Logger::warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'eye-care' ),
							$this->version,
	...
	protected function log_error( WP_Error $error ) {
		Logger::warning( $error->get_error_message() );
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'eye-care' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				Logger::warning( __( 'Invalid author mapping', 'eye-care' ) );
				Logger::debug( var_export( $map, true ) );
				continue;
			}
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'eye-care' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			Logger::warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'eye-care' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			Logger::info( sprintf(
				__( '%s "%s" already exists.', 'eye-care' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				Logger::notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'eye-care' ),
					$data['post_title']
				) );
				return false;
			}
		...
		if ( is_wp_error( $post_id ) ) {
			Logger::error( sprintf(
				__( 'Failed to import "%s" (%s)', 'eye-care' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		Logger::info( sprintf(
			__( 'Imported "%s" (%s)', 'eye-care' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		Logger::debug( sprintf(
			__( 'Post %d remapped to %d', 'eye-care' ),
			$original_id,
			$post_id
		) );
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								Logger::warning( sprintf(
									esc_html__( 'Failed to import term: %s - %s', 'eye-care' ),
									esc_html( $taxonomy ),
									esc_html( $term['name'] )
								) );
								continue;
	...
	protected function process_menu_item_meta( $post_id, $data, $meta ) {
		...
		Logger::debug( sprintf( 'Processing menu item %s', $item_type ) );
		...
		switch ( $item_type ) {
			...
			default:
				...
				Logger::debug( 'Unknown menu item type' );
				break;
		}
		...
		Logger::debug( sprintf( 'Menu item %d mapped to %d', $original_object_id, $object_id ) );
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			Logger::error( sprintf(
				__( 'Failed to import user "%s"', 'eye-care' ),
				$userdata['user_login']
			) );
			Logger::debug( $user_id->get_error_message() );
		...
		Logger::info( sprintf(
			__( 'Imported user "%s"', 'eye-care' ),
			$userdata['user_login']
		) );
		Logger::debug( sprintf(
			__( 'User %d remapped to %d', 'eye-care' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		...
		Logger::debug( sprintf(
			__( 'Term %d remapped to %d', 'eye-care' ),
			$original_id,
			$term_id
		) );
	...
	protected function process_term_meta( $meta, $term_id, $term ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					Logger::warning( sprintf(
						__( 'Failed to add metakey: %s, metavalue: %s to term_id: %d', 'eye-care' ),
						$key,
						$log_value,
						$term_id
					) );
					...
					Logger::debug( sprintf(
						__( 'Meta for term_id %d : %s => %s ; successfully added!', 'eye-care' ),
						$term_id,
						$key,
						$log_value
					) );
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'eye-care' ),
					$code,
		...
		// 	unlink( $upload['file'] );
		// 	return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'eye-care' ) );
		// }

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'eye-care' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'eye-care' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			Logger::debug( sprintf(
				...
				__( 'Running post-processing for post %d', 'eye-care' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					Logger::warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)', 'eye-care' ),
						get_the_title( $post_id ),
						$post_id
					) );
					Logger::debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found', 'eye-care' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					Logger::warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)', 'eye-care' ),
						get_the_title( $post_id ),
						$post_id
					) );
					Logger::debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found', 'eye-care' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				Logger::debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'eye-care' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				Logger::warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data', 'eye-care' ),
					get_the_title( $post_id ),
					$post_id
				) );
				Logger::debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			Logger::warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)', 'eye-care' ),
				get_the_title( $post_id ),
				$post_id
			) );
			Logger::debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'eye-care' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					Logger::warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'eye-care' ),
						$comment_id
					) );
					Logger::debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found', 'eye-care' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					Logger::warning( sprintf(
						__( 'Could not find the author for comment #%d', 'eye-care' ),
						$comment_id
					) );
					Logger::debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found', 'eye-care' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				Logger::warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'eye-care' ),
					$comment_id
				) );
				continue;
			}
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if( empty( $termid ) || ! is_numeric( $termid ) ) {
				Logger::warning( sprintf(
					__( 'Faulty term_id provided in terms-to-be-remapped array %s', 'eye-care' ),
					$termid
					) );
				continue;
			}
			...
			if( empty( $term_taxonomy ) ){
				Logger::warning( sprintf(
					__( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'eye-care' ),
					$term_id
					) );
				continue;
			}
			...
			if ( empty( $parent_slug ) ) {
				Logger::warning( sprintf(
					__( 'No parent_slug identified in remapping-array for term: %d', 'eye-care' ),
					$term_id
					) );
				continue;
			}
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				Logger::warning( sprintf(
					__( 'The term(%d)"s parent_slug (%s) is not found in the remapping-array.', 'eye-care' ),
					$term_id,
					$parent_slug
					) );
				continue;
			...
			if ( empty( $termattributes ) ) {
				Logger::warning( sprintf(
					__( 'No data returned by get_term_by for term_id #%d', 'eye-care' ),
					$term_id
					) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				Logger::warning( sprintf(
					__( 'Could not update "%s" (term #%d) with mapped data', 'eye-care' ),
					$termattributes['name'],
					$term_id
				) );
				Logger::debug( $result->get_error_message() );
			...
			delete_term_meta( $term_id, '_wxr_import_parent' );
			Logger::debug( sprintf(
				__( 'Term %d was successfully updated with parent %d', 'eye-care' ),
				$term_id,
				$mapped_parent
			) );
		}
	...
	function remap_featured_images() {
		...
		Logger::info( esc_html__( 'Starting remapping of featured images', 'eye-care' ) );
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					Logger::info( sprintf( esc_html__( 'Remapping featured image ID %d to new ID %d for post ID %d', 'eye-care' ), $value, $new_id, $post_id ) );
