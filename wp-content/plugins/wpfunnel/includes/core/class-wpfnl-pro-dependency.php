...
if ( !class_exists( 'Wpfnl_Pro_Dependency' ) ) {
    /**
    ...
    class Wpfnl_Pro_Dependency {
        ...
        public function __construct( $plugin, $root_file, $min_version = null, $text_domain = null ) {
            ...
            if ( 'plugins.php' === basename( $_SERVER['PHP_SELF'] ) && ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
                ...
                add_filter( 'plugin_action_links_' . plugin_basename( $root_file ), array( $this, 'plugin_action_links_maybe_deactivate' ) );
                add_filter( 'network_admin_plugin_action_links_' . plugin_basename( $root_file ), array( $this, 'plugin_action_links_maybe_deactivate' ) );
                ...
                add_action( 'after_plugin_row_' . plugin_basename( $root_file ), array( $this, 'plugin_row' ), 999 );
        ...
        private function set_message( $type ) {
            ...
            switch ( $type ) {
                case 'deactivate':
                    ...
                    $this->message = __( 'You need to Install & Activate the latest version of WPFunnels Basic(Free) before activating WPFunnels - Pro.', $this->text_domain );
                    break;
                case 'upgrade':
                case 'update':
                case 'activate':
        ...
        private function get_plugin_data( $attr = null, $plugin = null ) {
            ...
            if( !array_key_exists( $this->plugin, $installed_plugins ) || !in_array( $this->plugin, $installed_plugins, true ) ){
                return array();
            }
        ...
        public static function deactivate_self( $file, $network_wide = false ) {
            ...
            // register_activation_hook($this->root_file, array( $this, 'plugin_action_links_maybe_deactivate' ));
        }
        /**
        ...
        public function is_active() {
            ...
            if( !isset($installed_plugins['wpfunnels/wpfnl.php']) && !is_plugin_active( 'wpfunnels/wpfnl.php' ) ){
                ...
                $plugins = array(
                    array(
                        'name'    => 'wpfunnels',
                        'path'    => 'https://downloads.wordpress.org/plugin/'.$plugin_slug.'.latest-stable.zip',
                        'install' => 'wpfunnels/wpfnl.php'
                    )
                );
                return true;
            }
        ...
        public function plugin_row() {
            if ( ! $this->is_active()  ) {
                printf( '<tr class="plugin-update-tr"><td colspan="5" class="plugin-update"><div class="update-message" style="background-color: #ffebe8;">%s</div></td></tr>', $this->get_message() );
            }
        }
    }
}
