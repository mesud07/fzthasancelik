<?php
namespace EyeCareSpace\TemplateFunctions;
...
class Heading_Elements {
	...
	public static function get_page_title( $context = true ) {
		...
		if ( is_singular() ) {
			...
			$title = sprintf( esc_html__( 'Search Results for: %s', 'eye-care' ), get_search_query() );

			if ( get_query_var( 'paged' ) ) {
				/* translators: Paged search query title part. 1: Page number */
				$title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'eye-care' ), get_query_var( 'paged' ) );
			}
		} elseif ( ! $context ) {
			if ( is_category() ) {
				...
				$title = get_the_date( _x( 'Y', 'yearly archives date format', 'eye-care' ) );
			} elseif ( is_month() ) {
				$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'eye-care' ) );
			} elseif ( is_day() ) {
				$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'eye-care' ) );
	...
	public static function get_page_breadcrumbs( $sep_text = ' / ' ) {
		...
		if ( '' !== $out ) {
			...
			$out .= '<span>' . esc_html__( 'Search results for', 'eye-care' ) . ': "' . esc_html( get_search_query() ) . '"</span>';
			...
			if ( is_tax( 'post_format', 'post-format-gallery' ) ) {
				$out .= '<span>' . esc_html_x( 'Galleries', 'post format archive title', 'eye-care' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
				$out .= '<span>' . esc_html_x( 'Images', 'post format archive title', 'eye-care' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
				$out .= '<span>' . esc_html_x( 'Videos', 'post format archive title', 'eye-care' ) . '</span>';
			} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
				$out .= '<span>' . esc_html_x( 'Audio', 'post format archive title', 'eye-care' ) . '</span>';
			}
		} elseif ( is_post_type_archive() ) {
			...
			$out .= '<span>' . esc_html__( 'No breadcrumbs', 'eye-care' ) . '</span>';
		}

		$out = apply_filters( 'cmsmasters_page_breadcrumbs_filter_after', $out );

		return '<a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'eye-care' ) . '</a>' . $sep . $out;
	}
