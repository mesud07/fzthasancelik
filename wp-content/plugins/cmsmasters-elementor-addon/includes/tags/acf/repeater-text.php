<?php
namespace CmsmastersElementor\Tags\ACF;

use CmsmastersElementor\Acf_Utils;
use CmsmastersElementor\Tags\ACF\Text;


if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly
}


/**
 * Addon repeater text.
 *
 * Retrieves repeater text field data from an advanced custom field.
 *
 * @since 1.2.0
 */
class Repeater_Text extends Text {

	/**
	* Get tag name.
	*
	* Returns the name of the dynamic tag.
	*
	* @since 1.2.0
	*
	* @return string Tag name.
	*/
	public static function tag_name() {
		return 'repeater-' . parent::tag_name();
	}

	/**
	* Get tag title.
	*
	* Returns the title of the dynamic tag.
	*
	* @since 1.2.0
	*
	* @return string Tag title.
	*/
	public static function tag_title() {
		return __( 'Repeater Field', 'cmsmasters-elementor' );
	}

	/**
	* Register controls.
	*
	* Registers the controls of the dynamic tag.
	*
	* @since 1.2.0
	*/
	protected function register_controls() {
		Acf_Utils::add_key_control( $this, true );
	}

	/**
	* Tag render.
	*
	* Prints out the value of the dynamic tag.
	*
	* @since 1.2.0
	*/
	public function render() {
		list( $field, $repeater_values, $meta_key ) = array_pad( Acf_Utils::get_key_field( $this ), 3, false );

		$values_array = array();

		if ( $field && ! empty( $field['type'] ) ) {
			foreach ( $repeater_values as $value ) {
				switch ( $field['type'] ) {
					case 'radio':
						if ( isset( $field['choices'][ $value ] ) ) {
							$values_array[] = $field['choices'][ $value ];
						}

						break;
					case 'select':
						$values = (array) $value;

						foreach ( $values as $key => $item ) {
							if ( isset( $field['choices'][ $item ] ) ) {
								$values[ $key ] = $field['choices'][ $item ];
							}
						}

						$values_array[] = implode( ', ', $values );

						break;
					case 'checkbox':
						$values = array();

						foreach ( (array) $value as $item ) {
							if ( isset( $field['choices'][ $item ] ) ) {
								$values[] = $field['choices'][ $item ];
							} else {
								$values[] = $item;
							}
						}

						$values_array[] = implode( ', ', $values );

						break;
					case 'oembed':
						$values_array[] = $this->get_queried_object_meta( $meta_key );

						break;
					case 'google_map':
						$meta = $this->get_queried_object_meta( $meta_key );
						$values_array[] = isset( $meta['address'] ) ? $meta['address'] : '';

						break;
					default:
						$values_array[] = $value;
				}
			}
		}

		echo wp_json_encode( array_map(
			function( $result_item ) {
				return wp_kses_post( $result_item );
			},
			$values_array
		) );
	}

}
