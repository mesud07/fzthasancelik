...
use EyeCareSpace\TemplateFunctions\Post_Elements;
...
if ( is_search() && ! have_posts() ) {
	echo '<div class="' . $parent_class . '__no-posts">' .
		'<h3 class="' . $parent_class . '__no-posts-title">' . esc_html__( 'Nothing found', 'eye-care' ) . '</h3>' .
		'<div class="' . $parent_class . '__no-posts-description">' .
			'<p>' . esc_html__( 'Sorry, no posts matched your criteria. Please try another search.', 'eye-care' ) . '</p>' .
			'<p>' . esc_html__( 'You might want to consider some of our suggestions to get better results:', 'eye-care' ) . '</p>' .
			'<ul>' .
				'<li>' . esc_html__( 'Check your spelling.', 'eye-care' ) . '</li>' .
				'<li>' . esc_html__( 'Try a similar keyword, for example: tablet instead of laptop.', 'eye-care' ) . '</li>' .
				'<li>' . esc_html__( 'Try using more than one keyword.', 'eye-care' ) . '</li>' .
			'</ul>' .
		'</div>' .
	'</div>';
}
