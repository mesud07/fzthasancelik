...
abstract class Base_App {
	...
	const OPTION_NAME_PREFIX = 'cmsmasters_connect_';

	const SITE_URL = 'https://api.cmsmasters.net/connect/v1';

	const API_URL = 'https://api.cmsmasters.net/api/connect/v1';
	...
	public function render_admin_widget() {
		...
		if ( $this->is_connected() ) {
			...
			$title = sprintf( __( 'Connected as %s', 'cmsmasters-elementor' ), '<strong>' . $remote_user->email . '</strong>' );
			$label = __( 'Disconnect', 'cmsmasters-elementor' );
		...
		if ( current_user_can( 'manage_options' ) ) {
			printf( '<div><a href="%s">%s</a></div>', $this->get_admin_url( 'reset' ), __( 'Reset Data', 'cmsmasters-elementor' ) );
		}
	...
	public function action_authorize() {
		if ( $this->is_connected() ) {
			$this->add_notice( __( 'Already connected.', 'cmsmasters-elementor' ), 'info' );
	...
	public function action_get_token() {
		...
		$this->add_notice( __( 'Connected Successfully.', 'cmsmasters-elementor' ) );
	...
	public function action_disconnect() {
		if ( $this->is_connected() ) {
			$this->disconnect();
			$this->add_notice( __( 'Disconnected Successfully.', 'cmsmasters-elementor' ) );
		}
	...
	protected function print_notices( $notices ) {
		switch ( $this->auth_mode ) {
			...
			default:
				...
				echo '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' . __( 'Dismiss', 'cmsmasters-elementor' ) . '</span></button></div>';
		}
	}
